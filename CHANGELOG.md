...
- **New agent groups start without Claude Code's agent-teams and Workflow features** (~20% smaller per-turn context, live-measured: ~38,900 → ~30,300 tokens on a fresh session). Both overlap NanoClaw's own systems — agent teams duplicates a2a messaging (`create_agent` + destinations) and multiplies separately-billed agents invisibly; the Workflow tool duplicates NanoClaw's orchestration and is the single largest tool schema on every request. `DesignSync` and `ReportFindings` (desktop/UI-only tools that cannot work headless) are additionally blocked for all groups. **Existing groups are untouched** — their `settings.json` already exists and is never regenerated. To re-enable either feature for one group, edit that group's `data/v2-sessions/<id>/.claude-shared/settings.json` (remove `disableWorkflows` / add `"CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS": "1"` under `env`) and `ncl groups restart` — the edit sticks.
